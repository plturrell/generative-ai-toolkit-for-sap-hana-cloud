...
class IntermittentForecastInput(BaseModel):
    ...
    p: Optional[int] = Field(description="the smoothing parameter for demand, it is optional")
    q: Optional[int] = Field(description="the smoothing parameter for the time-intervals between intermittent demands, it is optional")
    forecast_num: Optional[int] = Field(description="the number of forecast steps, it is optional")
    optimizer: Optional[str] = Field(description="the optimizer for the model chosen from {'lbfgsb', 'brute', 'sim_annealing'}, it is optional")
    method: Optional[str] = Field(description="the method for the output chosen from {'sporadic', 'constant'}, it is optional")
    grid_size: Optional[int] = Field(description="specifies the number of steps from the start point to the length of data for grid search, it is optional")
    optimize_step: Optional[int] = Field(description="specifies the minimum step for each iteration of LBFGSB method, it is optional")
    accuracy_measure: Union[Optional[str], Optional[list]] = Field(description="the metric to quantify how well a model fits input data chosen from 'mse', 'rmse', 'mae', 'mape', 'smape', 'mase', it is optional")
    ignore_zero: Optional[bool] = Field(description="whether to ignore zero values in the dataset to calculate mape, it is optional")
    expost_flag: Optional[bool] = Field(description="whether to output the expost forecast, it is optional")
    thread_ratio: Optional[float] = Field(description="the ratio of threads to use for parallel processing, it is optional")
    iter_count: Optional[int] = Field(description="a positive integer that controls the iteration of the simulated annealing, it is optional")
    random_state: Optional[int] = Field(description="specifies the seed for random number generator and valid for Simulated annealing method, it is optional")
    penalty: Optional[float] = Field(description="a penalty is applied to the cost function to avoid over-fitting, it is optional")
